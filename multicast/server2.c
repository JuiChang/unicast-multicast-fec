/*  
Run the clients before run this server:
	./<exe> <filename>
*/
...
int send_file(int sockfd, struct sockaddr_in groupSock, char *file_name) {
    ...
    if (n < 0) error("ERROR : sendto()"); 
    n = sendto(sockfd, &num_full_chunk, sizeof(num_full_chunk), 0, (struct sockaddr*)&groupSock, sizeof(groupSock));
    if (n < 0) error("ERROR : sendto()"); 
    n = sendto(sockfd, &offset, sizeof(offset), 0, (struct sockaddr*)&groupSock, sizeof(groupSock));
    if (n < 0) error("ERROR : sendto()");
